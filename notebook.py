...
def open_file():
    file_path = filedialog.askopenfilename(title='Choose', filetypes=(('Text documents (*.txt)', '*.txt'), ('All files', '*.*')))
    if file_path:
...
main_menu.add_cascade(label='Wiev', menu=view_menu)
...
view_colors = {
    'dark': {
        'text_bg': 'black', 'text_fg': 'red', 'cursor': 'brown', 'select_bg': '#8D917A'
    },
    'light': {
